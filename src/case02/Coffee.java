...
public class Coffee {
	...
	// 建立一個內部類來代表一種咖啡類型的判斷規則,coffeerule是屬於咖啡才可以使用
	static class CoffeeRule {
		...
		public boolean matches(Double milk, Double coffee) {
			return condition.test(milk, coffee); // BiPredicate的test方法
		}
	...
	static {
		// 初始化規則->下面的東西都會帶入這些東西
		rules.add(new CoffeeRule("濃郁的拿鐵", "牛奶的比例遠高於咖啡，味道偏向牛奶濃郁的拿鐵。", (milk, coffee) -> milk >= 3 * coffee));
